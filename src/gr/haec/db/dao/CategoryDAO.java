...
public class CategoryDAO extends BaseDAO<Category> {
	...
	public CategoryDAO(Connection conn) throws SQLException {
		...
		selectByIdStatement = dbConnection.prepareStatement(
				"SELECT terms.term_id, terms.name, terms.slug, terms.term_group FROM wp_terms terms"
		...
		selectAllStatement = dbConnection.prepareStatement(
				...
				ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
		countStatement = dbConnection.prepareStatement(
				"SELECT count(*) FROM wp_terms terms"
						+ " JOIN wp_term_taxonomy terms_taxonomy ON terms.term_id = terms_taxonomy.term_id "
						+ "WHERE terms_taxonomy.taxonomy = 'category'",
				ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
	}
